...
void InitPosEmb(TIntV& Vocab, const int& Dimensions, TRnd& Rnd, TVVec<TFlt, int64>& SynPos, TIntFltVH& InitEmbeddingsHV, const bool& CustomDefault, TFltV& DefaultEmbeddingV, TFltV& EmbeddingVariabilityV, TIntIntH& RnmBackH)
{
	...
	for (int64 i = 0; i < SynPos.GetXDim(); i++)	//loop nodes/words
	{
		...
		bool HaveInitialEmbedding = false;
		TFltV CurrV;
		if (InitEmbeddingsHV.IsKey(orig_id))
		{
			HaveInitialEmbedding = true;
			CurrV = InitEmbeddingsHV.GetDat(orig_id);
			//printf("copy \n");
		}		
		for (int j = 0; j < SynPos.GetYDim(); j++) 		//loop embedding dimensions
		{
			//if have initial embedding for this position, use it
			if (HaveInitialEmbedding)
			{
				SynPos(i,j) = CurrV[j];		
			}
			//if no initial embedding, but custom default/init values, use those
...
void LearnEmbeddings(TVVec<TInt, int64>& WalksVV, const int& Dimensions,
	...
	*/
	...
	InitPosEmb(Vocab, Dimensions, Rnd, SynPos, InitEmbeddingsHV, CustomDefault, DefaultEmbeddingV, EmbeddingVariabilityV, RnmBackH);		//init embedding with random values - but not random anymore!
